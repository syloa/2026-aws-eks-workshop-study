...
<script>
  if (!window.location.pathname.includes('2026-aws-eks-workshop-study')) {
    const destination = "https://syloa.github.io/2026-aws-eks-workshop-study/kubernetes/eks/aews/aews-week01/";
    window.location.replace(destination);
  }
</script>

<noscript>
  <meta http-equiv="refresh" content="0; url=https://syloa.github.io/2026-aws-eks-workshop-study/kubernetes/eks/aews/aews-week01/">
</noscript>
...
쿠버네티스는 호스트(물리적 머신, 가상 머신)에서 수백 개, 수천 개 단위까지의 대규모 컨테이너 기반 워크로드를 운영하는 데 도움을 주는 컨테이너 오케스트레이션 소프트웨어입니다. 

> 여러 호스트에 분산되어있는 애플리케이션 컨테이너를 마치 한 곳에 있는 것처럼 관리할 수 있습니다. 
...
- **etcd**: [etcd](https://etcd.io/)는 쿠버네티스 클러스터와 관련된 모든 구성 데이터, 상태 데이터 및 메타데이터를 관리하는데 사용되는 key-value 형태의 저장소입니다. 쿠버네티스 클러스터의 모든 정보가 저장되는 일관된 단일 진실 원천(Single Source of Truth)의 역할을 합니다. 클러스터 데이터는 노드나 전체 클러스터가 재시작 되더라도 재구성될 수 있도록 지속적으로 유지됩니다. 

- **kube-scheduler**: 워커노드에 아직 할당(Binding)되지 않은 새로운 Pod를 찾고, 각 Pod를 가장 적절한 워커 노드에 할당하는 백그라운드 프로세스입니다.

- **kube-controller-manager**: 클러스터의 현재 상태를 모니터링하고, 사용자가 정의한 원하는 상태(Desired state)와 일치하도록 변경 사항을 적용합니다. 예를 들어, 기존 쿠버네티스 객체(Pod, ReplicaSet, Deployment 등)의 구성이 변경되면 Controller manager가 이를 감지하고 전환을 시도합니다. `Deployment controller`, `Node controller`, `ServiceAccount controller` 등이 있습니다.

- **cloud-controller-manager**: 클러스터를 퍼블릭 클라우드(AWS, GCP, Azure 등)의 API와 연결하는 컴포넌트입니다.
...
- EKS는 AWS가 컨트롤 플레인을 관리합니다. 사용자는 AWS가 제공하는 API 서버 엔드포인트(Private/Public DNS)를 kubectl로 접근하는 것만 가능합니다. API 서버 VM이나 etcd에 직접 접근이 불가합니다. [CloudWatch control plane logs](https://docs.aws.amazon.com/eks/latest/userguide/control-plane-logs.html)를 활성화해두면 로그를 통해 문제를 파악할 수 있으며, AWS가 관리하는 컨트롤 플레인 영역의 문제는 AWS Support의 지원이 필요합니다.
...
Hands-on은 제공된 Terraform 예제 코드를 통해 배포하였습니다.
...
# 자격증명 설정
aws eks update-kubeconfig --region ap-southeast-1 --name myeks
...
3) 테라폼 모듈 버전 정보 확인
테라폼에서 레지스트리의 모듈을 가져올 때는 코드 호환성을 유지하기 위해 버전 제약 조건([Version Constraints](https://developer.hashicorp.com/terraform/language/expressions/version-constraints))을 설정할 수 있습니다. 
...
3) 모든 포드의 컨테이너 이미지 정보 확인 : dkr.ecr 저장소 확인!
```bash
...
```
로컬 환경에서 `kubectl` 커맨드로 조회 시도 시 호스트 조회에 실패합니다. 
...
10.0.20.173
```
로컬 환경에서 EKS DNS 질의 테스트 시 Public API Endpoint와 다르게 사설 IP 대역이 반환됩니다. 
