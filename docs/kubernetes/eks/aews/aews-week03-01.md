...
<script>
  if (!window.location.pathname.includes('2026-aws-eks-workshop-study')) {
    const destination = "https://syloa.github.io/2026-aws-eks-workshop-study/kubernetes/eks/aews/aews-week03-01/";
    window.location.replace(destination);
  }
</script>

<noscript>
  <meta http-equiv="refresh" content="0; url=https://syloa.github.io/2026-aws-eks-workshop-study/kubernetes/eks/aews/aews-week03-01/">
</noscript>
...
1. 파드 레벨 스케일링
      ...
      4. CPA(Cluster Propotional Autoscaler): 클러스터 노드 수 또는 코어 수에 비례해 특정 워크로드의 파드 수를 조정
2. 노드 레벨 스케일링
...
| Object | K8s의 특정 오브젝트<br/>(예: Ingress, Service 등)메트릭 | Ingress의 RPS(초당 요청 수)에 따른 스케일링 | Custom Metrics API </br>(예: Prometheus Adapter) |
...
CPA는 클러스터의 총 코어 수 또는 노드 수에 비례해 대상 워크로드의 복제본 수를 자동 조정하는 오토스케일러입니다. `CoreDNS`처럼 클러스터 규모가 커질수록 처리량이 함께 증가하는 시스템 컴포넌트에 특히 적합합니다.

### 동작 방식

![](images/i42g-ramn4th42.png)


CPA Binary는 Deployment 객체로 `kube-system` 네임스페이스에 파드로 생성되며, 설정은 ConfigMap에 저장됩니다.

#### 주요 컴포넌트

- Poll Period: 이벤트 기반이 아닌 Polling 방식으로 동작합니다. (기본값: 10초)
- ConfigMap Watcher: 사용자가 정의한 스케일링 정책이 담긴 ConfigMap을 감시합니다. 정책이 변경되면 즉시 메모리에 반영합니다.
- Cluster Size Provider: 쿠버네티스 API 서버를 호출하여 현재 클러스터의 정보(Schedulable Nodes, 
- Reconciler: 현재 상태와 정책을 비교하여 최종적으로 유지해야 할 Target Replicas를 결정합니다.

#### CPA에 필요한 RBAC 요소

| 컴포넌트 | 설명 | 
| --- | --- |
| ServiceAccount | CPA 파드가 API 서버 인증시 사용하는 계정 | 
| ClusterRole | nodes를 get/list/watch 할 수 있는 권한과, 대상 리소스(예: deployments/scale)를 get/update 할 수 있는 권한 | 

### 동작 모드 

- **1. Linear Mode (선형 모드)**

Linear 모드는 코어 기준과 노드 기준으로 각각 필요한 복제본 수를 계산한 뒤, **더 큰 값**을 선택해 적용합니다.
각 기준 계산 시 올림을 적용하고, 마지막에 최소/최대 복제본 범위로 제한합니다.

가장 많이 사용되는 방식으로, 코어 수나 노드 수에 따라 일정한 비율로 파드를 늘립니다. 계산식은 다음과 같습니다.
> Replicas = clamp(max(ceil(cores/coresPerReplica), ceil(nodes/nodesPerReplica)), min, max)
 
- 설정 변수: 
    - `coresPerReplica`: 파드 1개당 할당될 코어 수 (예: 16코어당 파드 1개)
    - `nodesPerReplica`: 파드 1개당 할당될 노드 수 (예: 2노드당 파드 1개)
    - `min`: 최소 파드 수, 계산된 값이 min보다 작으면 min으로 제한 (가용성 관점에서 2 이상을 권장)
    - `max`: 최대 파드 수, 계산된 값이 max보다 크면 max로 제한
    - `preventSinglePointFailure`: 노드가 2개 이상일 때 파드도 최소 2개 이상 유지하도록 보정
    - 예시: 만약 nodesPerReplica가 2이고 현재 노드가 10대라면, CPA는 10 / 2 = 5개의 파드를 유지


- **2. Ladder Mode (계단형 모드)**
사전에 정의한 구간 표에 따라 파드 수를 계단식으로 결정하여 할당하는 방식입니다. 급격한 스케일의 변동을 줄이고 운영자가 의도한 단계별 확장 정책을 적용할 때 유용합니다.
...
EKS에서 제공되는 CoreDNS add-on은 클러스터 규모에 비례하여 파드 개수를 스스케일링하는 기능을 지원합니다.
Linear 모드로 작동하며 CPU 코어 256개당 1개 또는 노드 16개당 1개의 기준 중 더 큰 값을 따라갑니다.
계산방식은 다음과 같습니다.
> Replicas = clamp(max(ceil(core 수/256), ceil(node 수/16)), min, max)

Optional Configuration settings의 autoScaling 항목에서 최소/최대 Replica의 범위를 지정해 운영할 수 있습니다.
아래 명령어로 EKS CoreDNS Add-on의 스키마를 확인해볼 수 있습니다.

<details><summary>aws eks describe-addon-configuration --addon-name coredns --addon-version v1.13.2-eksbuild.4 --query "configurationSchema" --output text | jq</summary>
```bash
{
  ...
  "definitions": {
    "Coredns": {
      ...
      "properties": {
        ...
        "trafficDistribution": {
          "description": "Traffic distribution preference for routing to topologically closer endpoints. PreferSameZone routes to endpoints in the same zone; PreferSameNode routes to endpoints on the same node. If unset, uses default routing strategy.",
          "enum": [
            "PreferSameNode",
            "PreferSameZone",
            "PreferClose"
