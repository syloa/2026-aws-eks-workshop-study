...
<script>
  if (!window.location.pathname.includes('2026-aws-eks-workshop-study')) {
    const destination = "https://syloa.github.io/2026-aws-eks-workshop-study/kubernetes/eks/aews/aews-week04-01/";
    window.location.replace(destination);
  }
</script>

<noscript>
  <meta http-equiv="refresh" content="0; url=https://syloa.github.io/2026-aws-eks-workshop-study/kubernetes/eks/aews/aews-week04-01/">
</noscript>
...
EKS 보안 모델은 호출 대상(K8s API vs AWS API)과 요청의 방향에 따라 인증·인가 주체가 완전히 달라집니다. 

따라서 트러블슈팅 시에는 '누가(Identity)', '어떤 엔드포인트(API Server vs AWS Endpoint)'에 접근하는지를 정확히 특정해야 합니다.
...
EKS에서는 IAM, Kubernetes RBAC, ServiceAccount, STS, OIDC 같은 용어가 등장합니다. 이들이 모두 권한과 관련 있어 보이지만, 실제로는 서로 다른 경로에서 서로 다른 질문에 답합니다. 아래 세 문장은 전부 독립적인 문제입니다. 
...
EKS는 이 세 계층을 엮어주는 서비스입니다. 따라서 권한 오류가 발생했을 때도 **어느 영역에서 막혔는가**를 먼저 구분하면 원인 파악이 빠릅니다. 

이 흐름을 이해하면 다음과 같은 오해를 피할 수 있습니다.
...
### K8s API 리소스 구조 (Groups & Versions)
...
`kubectl api-resources` 명령어를 실행하면 현재 클러스터에 지원하는 모든 API 그룹과 버전을 확인할 수 있습니다. 

나타나는 `APIVERSION` 열은 해당 리소스가 어떤 그룹과 버전(Alpha, Beta, Stable)에 속해 있는지를 명시하며, 이는 YAML 매니페스트 작성 시 apiVersion 필드의 기준이 됩니다.
...
#### K8s의 신원 체계: User vs ServiceAccount
...
`kubectl`은 정적 토큰을 저장해두고 쓰는 방식이 아니라, 요청 시점마다 `aws eks get-token`을 실행해 **짧게 유효한 bearer token**을 생성합니다.

이 토큰은 사용자가 EKS에 AWS IAM principal로 인증(AuthN)할 수 있게 합니다. 

s`kubectl` 명령어는 일반적으로 다음과 같이 처리됩니다. 
...
#### `aws eks get-token` 내부 로직

- `cat ~/.kube/config`
    - ![](images/fi4/13/20261Nam461-i46g-ramn1st46-ramn1.png)

EKS용 kubeconfig를 확인하면 인증 섹션에 exec 명령어가 정의되어 있습니다. 

이는 kubectl이 요청을 보낼 때마다 실시간으로 `aws eks get-token`으로 **AWS STS**를 통해 서명된 토큰을 생성함을 의미합니다.

이 토큰은 임의의 문자열이 아닌 AWS STS `GetCallerIdentity` 호출을 사전 서명한 URL입니다. 내부적으로는 SigV4 서명 절차를 거쳐 생성됩니다.
...
운영자는 아래 로그를 통해 누가 로그인했는 지, 로그인 후 무엇을 했는 지를 확인할 수 있습니다.

EKS 환경에서는 컨트롤 플레인에 직접 접근할 수 없습니다. 로깅 기능을 활성화하여 CloudWatch Logs를 통해 컨트롤 플레인 로그 확인이 가능합니다. [Link](https://docs.aws.amazon.com/eks/latest/userguide/control-plane-logs.html)

![](images/fi4/13/20261Npm241-i24g-rpmn1st24-rpmn1.png)

- /aws/eks/'ClusterName'/cluster 로그 그룹에 저장됩니다.
![](images/fi4/13/20261Npm311-i31g-rpmn1st31-rpmn1.png)

1. authenticator 로그
      - 컨트롤 플레인이 IAM 자격 증명을 사용한 RBAC 인증을 수행한 내역을 확인할 수 있습니다.
2. audit 로그
...
#### Pod Identity를 못 쓰는 환경

EKS Pod Identity는 노드에 배포되는 Pod Identity Agent(DaemonSet, hostNetwork) 동작을 전제로 합니다. 

아래 환경에서는 현재 Pod Identity를 사용할 수 없고, IRSA를 사용해야 합니다.
...
### Terrafom EKS 모듈 + External Pod Identity 배포

- `도전과제`: 테라폼으로 EKS 모듈로 addon ExternalDNS 배포 시, ExternalDNS를 위한 Pod Identity 설정을 같이 배포해보기
...
- `kubectl logs -n external-dns <external dns pod 이름>`
...
### Pod Identity의 Session Policy

- `도전과제`: EKS Pod Identity를 사용하여 IAM Role에 Policy 추가 Session policies for Amazon EKS Pod Identity - [Link](https://aws.amazon.com/ko/blogs/containers/session-policies-for-amazon-eks-pod-identity/)

Pod Identity의 Session polic 기능은 추가 IAM 역할을 생성하지 않고도 Pod에 대한 IAM 권한을 세분화할 수 있는 방법입니다.

Session Policy가 적용된 Pod는 IAM Rolㄴe이 가진 전체 권한 중 세션 정책과 **교집합**인 권한만 사용 가능합니다.

> Session Policy는 Role 권한을 확장할 수 없고, 축소(제한)만 가능합니다

- **권한 계산 공식:**

```text
유효 권한 = IAM Role 권한 ∩ 세션 정책
```


- **S3 리소스 전체(*)에 대한 작업 권한을 가진 IAM 정책/역할 생성**
```bash
...
```
CreatePodIdentityAssociation 또는 UpdatePodIdentityAssociation API를 사용할 때 --session-policy 인자를 통해 JSON 형태의 정책을 전달할 수 있습니다.


kubectl create namespace test-ns
kubectl create serviceaccount s3-pod-identity-test -n test-ns

aws eks create-pod-identity-association \
    --cluster-name siyoung-eks \
    --namespace test-ns \
    --service-account s3-pod-identity-test \
    --role-arn arn:aws:iam::${AWS_ACCOUNT}:role/session-policy-demo-role \
    --region ${AWS_REGION}
```

- **연결 생성 후 s3 작업 수행 테스트**
...
# S3 Bucket 목록 불러오기 테스트
kubectl run s3-test --image=amazon/aws-cli:latest \
    --namespace=test-ns --rm -it --restart=Never \
...
make_bucket: session-policy-test-bucket-1776069715
pod "s3-test" deleted from test-ns namespace
```

- **S3 버킷 목록 리스팅만 허용하는 세션 정책 추가**
...
{
    "association": {
        "clusterName": "siyoung-eks",
        "namespace": "test-ns",
        "serviceAccount": "s3-pod-identity-test",
        "roleArn": "arn:aws:iam::{AccountId}:role/session-policy-demo-role",
        "associationArn": "arn:aws:eks:ap-southeast-1:{AccountId}:podidentityassociation/siyoung-eks/a-ba0kpocxfgmlalefh",
        "associationId": "a-ba0kpocxfgmlalefh",
        "tags": {},
        "createdAt": "2026-04-07T17:08:48.756000+09:00",
        "modifiedAt": "2026-04-07T17:43:53.002000+09:00",
...
> Session tag는 Session Policy와 함께 사용할 수 없습니다.
> 
> Session Policy를 사용할 때는 필수적으로 `--disable-session-tags`를 `true`로 설정하여아합니다.
> 
>  `--no-disable-session-tags` 설정 + Session Policy 적용 시 에러 반환
> aws: [ERROR]: An error occurred (InvalidParameterException) when calling the UpdatePodIdentityAssociation operation: When policy is specified, disableSessionTags must be set to true


- **Session Policy 적용 후 Create Bucket Access Denied**
    - "errorMessage": "User: arn:aws:sts::{AccountId}:assumed-role/session-policy-demo-role/eks-siyoung-ek-s3-test-dc6fe276-ae80-4d5d-aaae-51e5f2c66739 is not authorized to perform: s3:CreateBucket on resource: \"arn:aws:s3:::session-policy-test-bucket-2-1776069947\" **because no session policy allows the s3:CreateBucket action**"


<details><summary>CloudTrail 로그 전문</summary>
```
{
    "eventVersion": "1.11",
    "userIdentity": {
        "type": "AssumedRole",
        "principalId": "AROAWQOUVOKALXABM3EZS:eks-siyoung-ek-s3-test-dc6fe276-ae80-4d5d-aaae-51e5f2c66739",
        "arn": "arn:aws:sts::{AccountId}:assumed-role/session-policy-demo-role/eks-siyoung-ek-s3-test-dc6fe276-ae80-4d5d-aaae-51e5f2c66739",
        "accountId": "{AccountId}",
        "accessKeyId": "ASIAWQOUVOKAL6JVT2YQ",
        "sessionContext": {
            "sessionIssuer": {
                "type": "Role",
                "principalId": "AROAWQOUVOKALXABM3EZS",
                "arn": "arn:aws:iam::{AccountId}:role/session-policy-demo-role",
                "accountId": "{AccountId}",
                "userName": "session-policy-demo-role"
            },
            "attributes": {
                "creationDate": "2026-04-08T08:45:50Z",
                "mfaAuthenticated": "false"
            }
        }
    },
    "eventTime": "2026-04-08T08:45:51Z",
    "eventSource": "s3.amazonaws.com",
    "eventName": "CreateBucket",
    "awsRegion": "ap-southeast-1",
    "sourceIPAddress": "52.221.201.84",
    "userAgent": "[aws-cli/2.34.29 md/awscrt#0.31.2 ua/2.1 os/linux#6.12.77-99.140.amzn2023.x86_64 md/arch#x86_64 lang/python#3.14.3 md/pyimpl#CPython m/Z,E,b,z cfg/retry-mode#standard md/installer#docker md/distrib#amzn.2023 md/prompt#off md/command#s3.mb]",
    ...
    "requestParameters": {
        ...
        "bucketName": "session-policy-test-bucket-2-1776069947",
        "Host": "session-policy-test-bucket-2-1776069947.s3.ap-southeast-1.amazonaws.com"
    },
    ...
    "additionalEventData": {
        ...
        "x-amz-id-2": "JQU9oRsymZPJu4WRDOgzlC+YVPTRTMJEqoih9oVadHIiARXypmtH6L3Cq9tnJZF9Q/d9xcbI9tyUk9AufmWhp+qcrXY1nAzf",
        "bytesTransferredOut": 553
    },
    "requestID": "5F13VVKTZY22REPX",
    "eventID": "5632b81a-ddb1-4604-837d-8514437d34a8",
    "readOnly": false,
    ...
    "tlsDetails": {
        "tlsVersion": "TLSv1.3",
        "cipherSuite": "TLS_AES_128_GCM_SHA256",
        "clientProvidedHostHeader": "session-policy-test-bucket-2-1776069947.s3.ap-southeast-1.amazonaws.com"
    }
}
```
</details>

- **Cross Account 간 세션 정책**

Cross Account 간 권한 부여 시 세션 정책은 Target Account의 IAM Role 정책과 교집합인 권한을 가지게 됩니다.

```bash
Pod → (source role 수임) → (target role 수임) → Pod에게 target 자격증명 전달
                                                       ↑
                                          Pod는 target 자격증명의 권한과 Session Policy의 교집합 권한을 가짐

Source Role (Account A)
  └── IAM 권한1: 
        Action: sts:AssumeRole
                sts:TagSession
        Resource: arn:aws:iam::${ACCOUNT_B}:role/target-role
  └── IAM 권한2: 
        Action: s3:PutObject
        Resource: *

Target Role (Account B)  
  └── IAM 권한: 
        Action: s3:GetObject
                s3:PutObject
        Resource: *

Session Policy: s3:GetObject 허용, PutOjbect 제한
```